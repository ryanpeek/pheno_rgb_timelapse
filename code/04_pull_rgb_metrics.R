...
ggplot() +
  ...
  labs(title=glue("Excess Greeness (exG): {site_id}"),
       ...
       x="", caption="Data from RECONYX hourly camera") +
  geom_image(
    data = tibble(datetime = ymd_hms(glue("{photo_date_location}")), exG = 100),
    aes(x=datetime, y=exG, image = glue("{exif_path}/ROI/{site_id}_{mask_type}_roi_masked.png")), size=0.6)
...
ggplot() +
  ...
  labs(title=glue("Water (gbR): {site_id}"),
       ...
       x="", caption="Data from RECONYX hourly camera") +
  geom_image(
    data = tibble(datetime = ymd_hms(glue("{photo_date_location}")), exG = 2.35),
    aes(x=datetime, y=exG, image = glue("{exif_path}/ROI/{site_id}_{mask_type}_roi_masked.png")), size=0.5)

ggsave(glue("figs/gbR_{site_id}_{mask_type}_midday.png"), width = 10, height = 8, dpi = 300, bg = "white")
